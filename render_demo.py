...
# On native Windows the Node tooling is exposed as wrapper scripts / shims
# (``npm.cmd``, ``npm.exe``). On Linux and macOS -- including WSL -- only the
# bare name is a real executable; a Windows ``npm.cmd`` reachable through
# ``/mnt/c`` would be selected first and then fail with "Exec format error"
# when executed by the Linux kernel.
...
def command_candidates(base: str) -> tuple[str, ...]:
    """Return executable-name candidates for ``base`` ordered for this OS.

    Windows must search the wrapper/shim variants (``npm.cmd``, ``npm.exe``)
    first. Every other platform -- Linux, macOS and WSL -- searches the bare
    name *only*, so a Windows ``*.cmd`` exposed via ``/mnt/c`` is never picked
    (which would otherwise fail with "Exec format error").
    """
    if IS_WINDOWS:
